...
class ReportsController extends Controller
{
  ...
  public function timeSheetListByUser(Request $request)
  {
    ...
    $shifting_schedule = ShiftingSchedule::select(
      ShiftingSchedule::raw('DATE_FORMAT(morning_start, "%H:%i") as morning_start'),
    ...
    return datatables()->eloquent($query)
      ...
      ->editColumn('drafting_masters_id', function (Timesheet $timesheet) {

        return '<a  class="text-primary" href="' . route('timesheets.drafting', $timesheet->drafting_masters_id) . '"><u>' . $timesheet->drafting_masters_id . '</u></a>';
      })

      ->rawColumns(['user_id', 'type', 'drafting_masters_id'])
      ->toJson();
  }
  ...
  public function generateDrafting($request)
  {
    ...
    if ($request->status) {
      if ($request->status == "On-going") {
        ...
        $drafting = $drafting->whereIn('drafting_masters.status', [
          'Assigned', 'Ready For Check', 'Ready To Submit', 'Ready For Six Stars', 'In Six Stars'
        ]);
      } else {
