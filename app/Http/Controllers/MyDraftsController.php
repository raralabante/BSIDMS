...
class MyDraftsController extends Controller
{
  ...
  {
    return view('mydrafts.mydraftscompleted');
  }
  public function jobStopper()
  {
    Timesheet::leftJoin('job_drafting_status', 'timesheets.drafting_masters_id', '=', 'job_drafting_status.drafting_masters_id')
    ...
    JobDraftingStatus::where('user_id', Auth::user()->id)
      ->where('type', '=', 'DRAFTING')
      ->where('status', '=', '1')
      ->update(['status' => 0]);
  }
  ...
  public function myDraftsCompletedList(Request $request)
  {
    ...
    if ($request->ajax()) {
      $query = DraftingMaster::select(
        ...
        ->where('timesheets.user_id', '=', Auth::user()->id)
        ->where('timesheets.type', '=', 'DRAFTING')
        ->where('drafting_masters.status', '!=', 'Assigned')
        ->groupBy('drafting_masters.id');
      ...
      return datatables()->eloquent($query)
        ...
        ->rawColumns(['job_number', 'drafting_hours'])
        ->toJson();
    }
  }
